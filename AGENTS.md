...
- AI 头像按身份类型确定：云端分身以主人头像为圆形主图、既有 Eva Logo 为右下角小图，两者均不可修改；个人助理默认以主人头像为圆形主图，可自选 emoji／图标替换主图，同一 Eva Logo 为右下角小图；无主人的 AI（数字员工、项目管家、外部）沿用各自已确认的头像配置能力，保持单张圆形主图。所有 AI 统一调用 `window.EvaAIIdentity.avatar` 渲染。该头像是 AI 身份的全局属性：通讯录、消息、我的 AI、项目成员、选择器、详情和编辑预览必须读取同一份头像数据；React 与旧 HTML 只做渲染适配，禁止业务入口自行拼接或替换头像。
...
- 换头像只换个人助理的主图：默认沿用主人头像，可自选 emoji／图标替换，右下角角标恒为 Eva Logo；云端分身的名字与头像均由 ownerId 推导，不提供改名或换头像入口。
