...
- AI 头像按身份类型确定：云端分身固定使用既有 Eva Logo，不可修改；其他 AI 沿用各自已确认的头像配置能力。所有 AI 统一调用 `window.EvaAIIdentity.avatar` 渲染。该头像是 AI 身份的全局属性：通讯录、消息、我的 AI、项目成员、选择器、详情和编辑预览必须读取同一份头像数据；React 与旧 HTML 只做渲染适配，禁止业务入口自行拼接或替换头像。
...
- 用户可见的中文名称或界面文案中，中文与拉丁字母、英文缩写或阿拉伯数字相邻时，默认使用一个半角空格，例如「PPT 制作专家」「智驾 VLM 数据标注专家」「国际 2C 销售专家」。英文缩写内部、数字与英文组成的完整术语、版本号及规范内连字符不拆分，例如 `A2A`、`2C`、`ESER-AI`。
- 中文标点前后不添加西文空格。数字员工展示名不保留作者姓名、「某某版」「专业版」等导入尾巴，直接改写为稳定的能力名称；品牌确实影响服务范围时使用品牌前缀，例如「吉利客服服务专家」，不使用「客服专家-吉利」或括号后缀。
- 数字员工展示名必须语义完整且以「专家」结尾；禁止通过全局删除再追加「专家」来改名，禁止产生「专员专家」「工程师专家」「专家版专家」或暴露 `_cc-api` 等内部技术后缀。原始导入名、历史本地状态和新建名称必须收敛到同一公共规范化函数；人名或昵称不得作为展示名，改用实际能力，例如「数智支付专家」。
- 品牌固有写法和已经确认的产品名称优先于通用空格规则；新增例外必须在命名映射和自动化用例中显式登记，不能在单个页面临时替换。
- 修改批量身份数据、导入逻辑或名称迁移时，必须运行数字员工命名合同测试，覆盖全部预置展示名、历史别名及新建记录；检查专家后缀、混排空格、重复职业后缀、异常连字符、内部标识和首尾空白。
