...
- AI 头像按身份类型确定：云端分身默认使用既有 Eva Logo，主人可在身份资料卡更换；其他 AI 沿用各自已确认的头像配置能力。数字员工与项目管家头像固定，不提供更换入口。所有 AI 统一调用 `window.EvaAIIdentity.avatar` 渲染。该头像是 AI 身份的全局属性：通讯录、消息、我的 AI、项目成员、选择器、详情和编辑预览必须读取同一份头像数据；React 与旧 HTML 只做渲染适配，禁止业务入口自行拼接或替换头像。
...
- 每人零个或一个分身，名称固定为「xxx的 AI 分身」，由真人稳定身份推导；默认使用既有 Eva Logo，主人可通过身份资料卡更换分身头像。分身不可改名；个人助理、数字员工和项目管家不受单分身限制，其中个人助理可由主人更换头像，数字员工和项目管家头像固定。
