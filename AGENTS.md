...
- 「我的 AI」中的默认团队名称为「我的AI团队」，动态包含本人及全部可用 AI，不可编辑、不可解散；自定义 AI 团队保存成员快照，成员编辑与解散统一从当前团队父群会话右上角三点打开的聊天信息中进入，中栏团队行不保留第二套治理菜单。解散后删除该团队的消息、草稿、子区及未读状态，不影响 AI 单聊。消息、草稿及子区按团队隔离。团队群只响应被明确 @ 的 AI，AI 单聊直接发送即触发当前 AI。
