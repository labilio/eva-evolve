...
root.EvaChatSettings={create(ui,store){
 ...
 function memberIdentity(p){return p.identityAppearance?h('span',{className:'eva-members-human-identity'},root.EvaAIIdentity.avatar(p.identityAppearance,32,h),h('span',{className:'eva-identity-name-row'},h('span',{className:'eva-members-human-name eva-identity-name-text','data-eva-tooltip':p.name,'data-eva-tooltip-clamp':''},p.name),root.EvaAIIdentity.badge(h))):['project-agent','employee'].includes(p.kind)?h(ProjectAgentIdentity,{agent:p}):p.kind==='clone'?h(CloneIdentity,{clone:p}):h(HumanIdentity,{id:p.id});}
 // Group and thread lists share identity/role rendering; only parent groups supply actions.
 function MemberRow({p,role,onProfile,actions}){return h('div',{className:'eva-chat-member-list-row'},h('button',{type:'button',className:'eva-chat-member-profile',onClick:onProfile},memberIdentity(p),role&&h(Tag,{className:'eva-chat-management-role-tag',color:'grey',size:'small'},role)),actions&&h('div',{className:'eva-chat-member-actions'},actions));}
 // Freeze only display order for one list visit; identities and permissions remain live.
 function memberType(p){return p.kind==='human'?0:p.kind==='clone'?1:2;}
 function MemberFilters({members,type,setType}){return h('div',{className:'eva-chat-member-chips','aria-label':'成员类型筛选'},['全部','联系人','AI 分身','数字员工'].map((label,index)=>{const total=index?members.filter(p=>memberType(p)===index-1).length:members.length;const button=h(Button,{'aria-label':label+' '+total,'aria-pressed':type===index-1,theme:type===index-1?'light':'borderless',onClick:()=>setType(index-1)},h('span',null,label),h('span',{className:'eva-chat-member-chip-count'},' '+total));return h('span',{key:label,className:index===0?'eva-chat-member-chip-all':'eva-chat-member-chip-item'},button,index===0&&h('span',{className:'eva-chat-member-chip-divider','aria-hidden':true}));}));}

 function memberOrder(members,ownerId,managerIds,botAdminIds=[]){const rank=p=>memberType(p)*10+(p.id===ownerId?0:managerIds.includes(p.id)||botAdminIds.includes(p.id)?1:2);return [...members].sort((a,b)=>rank(a)-rank(b)).map(p=>p.id);}
 function applyMemberOrder(members,ids){const ranks=new Map(ids.map((id,index)=>[id,index]));return [...members].sort((a,b)=>(ranks.get(a.id)??ids.length)-(ranks.get(b.id)??ids.length));}
 function MemberAction({label,name,Icon,danger=false,onClick}){return h(Button,{className:'eva-chat-member-action',theme:'borderless',type:danger?'danger':'tertiary',size:'small',icon:h(Icon,{size:18,strokeWidth:1.75}),'aria-label':label+' '+name,'data-eva-tooltip':label,onClick});}
 ...
 function ChatSettings({channel,onClose,onManageProject,onClear,sessionInfoOnly=false,conversationActions,fixedGroupActions,projectScoped=false}){
  ...
  const members=fixed?channel.fixedMembers:allowed?store.groupMembers(id):[],humans=allowed?humanItems(g.humans.map(m=>({...store.person(m.id),...m})),undefined):[],ordered=fixed?members:[...humans.map(p=>({...p,kind:'human'})),...members.filter(p=>p.kind!=='human')];
  const person=store.person(channel.personId),bot=store.clone(channel.identityId),name=settings.name||channel.name,projectContext=group&&allowed?store.conversationContext(id,actor):null,normalizedMemberQuery=memberQuery.trim().normalize('NFKC').toLocaleLowerCase(),visibleMembers=applyMemberOrder(ordered,memberOrderIds).filter(p=>(memberTypeFilter===-1||memberType(p)===memberTypeFilter)).filter(p=>!normalizedMemberQuery||String(p.name||'').normalize('NFKC').toLocaleLowerCase().includes(normalizedMemberQuery));
  ...
  const role=p=>fixed?null:p.id===g?.ownerId?'群主':p.kind==='human'&&governance.managerIds.includes(p.id)?'群管理员':governance.botAdminIds.includes(p.id)?'Bot 管理员':null;
  const eligibleRemove=p=>['project-agent','employee'].includes(p.kind)?(manage||p.by===actor):p.kind==='clone'?(manage||p.ownerId===actor):(manage&&p.id!==g.ownerId&&p.id!==actor);
  const openFixedAction=action=>{onClose();requestAnimationFrame(action);};
  const ownerSuccessors=owner?humans.filter(p=>p.id!==actor):[];
  const leave=()=>{let outcome=null;if(!run(()=>{outcome=store.leaveGroup(id,actor);}))return;setConfirm(null);onClose();if(outcome?.type==='dissolved'&&Toast)Toast.success('群聊已解散');};
  ...
  return h('aside',{className:'eva-chat-settings','aria-label':'聊天信息管理',onKeyDown:e=>{if(e.key==='Escape'&&!picker&&!confirm&&!profile){e.stopPropagation();page==='main'?onClose():setPage('main');}}},
   ...
   h('div',{className:'eva-chat-settings-body'},error&&h('p',{role:'alert',className:'eva-members-error'},error),
    page==='main'&&h(R.Fragment,null,
     // 固定成员群（AI 小队父群）没有 store 里的群/项目记录，g 为空；ownerId 只在普通群里有意义。
     group?section(h('div',{className:'eva-chat-member-grid'},ordered.slice(0,19).map(p=>h(MemberTile,{key:p.id,p,onClick:()=>setProfile(p)})),(allowed&&!all||editableFixed)&&h('button',{type:'button',className:'eva-chat-member-add','aria-label':editableFixed?'添加 AI 小队成员':'添加群聊成员',onClick:editableFixed?()=>openFixedAction(fixedGroupActions.onEditMembers):()=>setPicker('add')},h(PlusIcon,{size:26}))),h(Row,{title:'查看全部 '+members.length+' 名成员',onClick:()=>{setMemberQuery('');setMemberTypeFilter(-1);setMemberOrderIds(fixed?ordered.map(p=>p.id):memberOrder(ordered,g?.ownerId,governance.managerIds,governance.botAdminIds));setPage('members');}})):
     ...
     projectContext&&section(h(ProjectRow,{context:projectContext,scoped:projectScoped})),
     ...
     fixedGroupActions&&section(h(EditRow,{title:'AI 小队名称',value:name,allowEmpty:false,onSave:fixedGroupActions.onRename})),
     fixedGroupActions&&section(h(Row,{title:'解散 AI 小队',danger:true,onClick:()=>openFixedAction(fixedGroupActions.onDissolve)})),
     prefRows,
   ...
   h(Modal,{className:'eva-members-modal',title:confirm==='clear'?'清空聊天记录':confirm==='leave'||confirm==='ownerLeave'?'退出群聊':confirm==='dissolve'?'解散群聊':confirm?.remove?'确认移除成员':'确认操作',visible:!!confirm,onCancel:()=>setConfirm(null),okText:confirm==='clear'?'清空':confirm==='leave'?'退出群聊':confirm==='dissolve'?'解散群聊':'确认',cancelText:'取消',okButtonProps:['clear','leave','dissolve'].includes(confirm)?{type:'danger'}:undefined,...(confirm==='ownerLeave'?{footer:ownerSuccessors.length?h('div',{className:'eva-group-owner-leave-footer'},h(Button,{type:'danger',theme:'light',onClick:()=>leave()},'退出群聊'),h('div',{className:'eva-group-owner-leave-footer-actions'},h(Button,{onClick:()=>setConfirm(null)},'取消'),h(Button,{theme:'solid',onClick:()=>{setConfirm(null);setPicker('transfer');}},'转让群主'))):h(R.Fragment,null,h(Button,{onClick:()=>setConfirm(null)},'取消'),h(Button,{type:'danger',theme:'solid',onClick:()=>leave()},'退出并解散'))}:{}),onOk:()=>{if(run(()=>{if(confirm==='clear')onClear();else if(confirm==='leave'){store.leaveGroup(id,actor);onClose();}else if(confirm==='dissolve'){store.dissolveGroup(id,actor);onClose();Toast&&Toast.success('群聊已解散');}else if(confirm?.remove){const p=confirm.remove;['project-agent','employee'].includes(p.kind)?store.removeEmployee(id,actor,p.id):p.kind==='clone'?store.removeClone(id,actor,p.id):store.remove(id,actor,p.id);}}))setConfirm(null);}},h('p',null,confirm==='clear'?'清空你在此设备的当前聊天记录，其他成员的记录不受影响。':confirm==='leave'?'退出后，你及你的分身将离开本群，并失去全部子区访问权。历史内容不会被改写。':confirm==='ownerLeave'?(ownerSuccessors.length?'请在退出前将群主转让给其他群成员。否则，群主将自动转让给群内其他可接任成员。':'群聊没有其他可接任成员。退出群聊将解散本群及全部子区，你将无法继续查看相关聊天记录。'):confirm==='dissolve'?'解散后，本群及全部子区将不再可访问。':confirm?.remove?'确认移除 '+confirm.remove.name+'？'+(confirm.remove.kind==='human'?(g?.projectId?'其分身及项目内群聊关系将一并移除。':'其分身也会离开本群。'):''):''),confirm?.remove&&identity(confirm.remove),error&&h('p',{role:'alert',className:'eva-members-error'},error)));
 }
 ...
 function ThreadMembers({groupId,actorId}){
  ...
  const threadRole=p=>p.id===g.ownerId?'群主':p.kind==='human'&&governance.managerIds.includes(p.id)?'群管理员':governance.botAdminIds.includes(p.id)?'Bot 管理员':null;
  if(allOpen){
   const normalized=memberQuery.trim().normalize('NFKC').toLocaleLowerCase();
   const visibleMembers=applyMemberOrder(ordered,memberOrderIds).filter(p=>(memberTypeFilter===-1||memberType(p)===memberTypeFilter)).filter(p=>!normalized||String(p.name||'').normalize('NFKC').toLocaleLowerCase().includes(normalized));
  ...
  return h(R.Fragment,null,
   h('section',{className:'eva-chat-setting-section'},
    h('div',{className:'eva-chat-member-grid'},ordered.slice(0,19).map(p=>h(MemberTile,{key:p.id,p,onClick:()=>setProfile(p)}))),
    h(Row,{title:'查看全部 '+members.length+' 名成员',onClick:()=>{setMemberQuery('');setMemberTypeFilter(-1);setMemberOrderIds(memberOrder(ordered,g.ownerId,governance.managerIds,governance.botAdminIds));setAllOpen(true);}})),
   h(IdentityCard,{identity:profile,onClose:()=>setProfile(null)}));
 }
 ChatSettings.ThreadMembers=ThreadMembers;
 ChatSettings.Row=Row;
 ChatSettings.ProjectRow=ProjectRow;
 return ChatSettings;
}};
})(window);
