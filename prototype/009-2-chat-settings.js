...
root.EvaChatSettings={create(ui,store){
 const {React:R,Button,Modal,Input,Switch,Tag,PlusIcon,CircleMinusIcon,CloseIcon,BackIcon,SearchIcon,HumanIdentity,CloneIdentity,ProjectAgentIdentity,MemberPicker,SinglePersonPicker,humanItems,cloneItems,useState,IdentityCard,ProjectIdentity,AvatarEditor,readAvatarFile,useNavigate,Toast}=ui,h=R.createElement;
  ...
  return h('aside',{className:'eva-chat-settings','aria-label':'聊天信息管理',onKeyDown:e=>{if(e.key==='Escape'&&!picker&&!confirm&&!profile){e.stopPropagation();page==='main'?onClose():setPage('main');}}},
   ...
   h('div',{className:'eva-chat-settings-body'},error&&h('p',{role:'alert',className:'eva-members-error'},error),
    page==='main'&&h(R.Fragment,null,
     ...
     group?section(h('div',{className:'eva-chat-member-grid'},ordered.slice(0,19).map(p=>h(MemberTile,{key:p.id,p,role:role(p),ownerId:g?.ownerId,onClick:()=>setProfile(p)})),(allowed&&!all||editableFixed)&&h('button',{type:'button',className:'eva-chat-member-add','aria-label':editableFixed?'添加 AI 小队成员':'添加群聊成员',onClick:editableFixed?()=>openFixedAction(fixedGroupActions.onEditMembers):()=>setPicker('add')},h(PlusIcon,{size:26}))),h(Row,{title:'查看全部 '+members.length+' 名成员',onClick:()=>{setMemberQuery('');setPage('members');}})):
