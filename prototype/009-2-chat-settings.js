...
root.EvaChatSettings={create(ui,store){
 ...
 function MemberFilters({members,type,setType}){return h('div',{className:'eva-chat-member-chips','aria-label':'成员类型筛选'},['全部','联系人','AI 分身','数字员工'].map((label,index)=>{const total=index?members.filter(p=>memberType(p)===index-1).length:members.length;return h(Button,{key:label,'aria-label':label+' '+total,'aria-pressed':type===index-1,theme:type===index-1?'light':'borderless',onClick:()=>setType(index-1)},label+' '+total);}));}
