(function(root){
  ...
  root.EvaMembersUI={create({React:R,Button,Select,Modal,Table,Input,Tag,Checkbox,Radio,Switch,PlusIcon,CloseIcon,BackIcon,SearchIcon,useNavigate},store,files){
    const h=R.createElement;
    function HumanIdentity({id,detail,compact=false}){const person=store.person(id);return h('span',{className:'eva-members-human-identity'+(compact?' is-compact':'')},h('img',{className:'eva-members-human-avatar',alt:'',src:root.EvaAvatar.personUri(id)}),h('span',{className:'eva-members-human-copy'},h('span',{className:'eva-members-human-name'},person?.name||id),detail&&h('span',{className:'eva-members-human-role'},detail)));}
        ...
        h('div',{className:'eva-member-picker'+(field?'':' eva-member-picker--selection-only')},
          ...
          h('div',{className:'eva-member-picker__members'+(field?'':' eva-member-picker__members--selection-only')},
            ...
            h('div',{className:'eva-member-picker__panel',...(field?{'aria-labelledby':'eva-member-picker-members-label'}:{'aria-label':memberLabel})},
              ...
              h('aside',{className:'eva-member-picker__selected','aria-label':'已选成员'},
    ...
    function SinglePersonPicker({visible,title,items,onCancel,onSubmit,submit='确认'}){
      ...
      const chosen=items.filter(item=>ids.includes(item.id)&&!item.disabled);
      return h(Modal,{className:'eva-members-modal eva-picker-modal',width:640,title,visible,onCancel,footer:null},
        h(SelectionBody,{items,selected:ids,onChange:setIds,single:true}),
        error&&h('p',{role:'alert',className:'eva-members-error'},error),
        h('div',{className:'eva-picker-footer'},h(Button,{onClick:onCancel},'取消'),h(Button,{theme:'solid',type:'primary',disabled:chosen.length!==1,onClick:()=>{try{onSubmit(chosen);}catch(e){setError(e.message);}}},submit)));
    }
    ...
    function MentionPicker({scopeId,members:sourceMembers,visible,query="",activeIndex=0,onActiveChange,onClose,onChoose}){
      ...
      const matched=members.filter(m=>m.name.normalize('NFKC').toLocaleLowerCase().includes(needle)).sort((a,b)=>Number(b.name.normalize('NFKC').toLocaleLowerCase().startsWith(needle))-Number(a.name.normalize('NFKC').toLocaleLowerCase().startsWith(needle)));
      let itemIndex=0;
      const groups=[{title:'人类成员',items:matched.filter(m=>m.kind==='human')},{title:'AI 成员',items:matched.filter(m=>m.kind!=='human')}].filter(group=>group.items.length);
      ...
      return h('section',{ref:panel,className:'eva-im-mention-picker',role:'dialog','aria-label':'提及成员',onKeyDown:e=>{if(e.key==='Escape'){e.preventDefault();e.stopPropagation();onClose();}if(e.key==='ArrowDown'||e.key==='ArrowUp'){const items=Array.from(panel.current.querySelectorAll('input,button'));const index=items.indexOf(document.activeElement);e.preventDefault();items[(index+(e.key==='ArrowDown'?1:-1)+items.length)%items.length]?.focus();}}},
        ...
        h('div',{className:'eva-im-mention-options'},!query&&h('button',{type:'button',...candidateProps(),onMouseDown:e=>e.preventDefault(),onClick:()=>choose('所有人','all')},h('span',{className:'eva-im-mention-all'},'@'),h('span',{className:'eva-im-mention-broadcast-copy'},'所有人',h('small',null,'通知会话中的人类成员'))),
    ...
    const ChatSettings=root.EvaChatSettings.create({React:R,Button,Modal,Input,Switch,PlusIcon,CloseIcon,BackIcon,HumanIdentity,CloneIdentity,ProjectAgentIdentity,MemberPicker,SinglePersonPicker,humanItems,cloneItems,useState,IdentityCard:cards.IdentityCard,useNavigate},store);
