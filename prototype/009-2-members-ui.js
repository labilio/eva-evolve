(function(root){
  ...
  root.EvaMembersUI={create({React:R,Button,Select,Modal,Table,Input,Tag,Checkbox,Radio,Switch,PlusIcon,CloseIcon,BackIcon,useNavigate},store,files){
    ...
    function humanItems(people,pid,groupHint=false){const s=store.snapshot(),p=s.projects[pid],rank={owner:0,admin:1,member:2};return people.filter(person=>store.person(person.id)).map(person=>{const role=p?.humans.find(m=>m.id===person.id)?.role;return {...person,kind:'human',projectRole:role,detail:roleNames[role]||'',selectionHint:groupHint&&['owner','admin'].includes(role)?'加入后拥有群管理权限':''};}).sort((a,b)=>(rank[a.projectRole]??3)-(rank[b.projectRole]??3));}
    ...
    function Members({scopeId}){
      ...
      return h('section',{className:'eva-members'},sid==='prod'&&new URLSearchParams(root.location.search).has('picker-preview')&&h(PickerPreview),h('div',{className:'eva-members-toolbar'},h('div',null,h('h3',null,isProject?'项目成员':'群聊成员'),h('p',{className:'eva-members-muted'},joined?`${scope.humans.length} 位人类 · ${scope.cloneIds.length} 个个人分身${store.projectAgent(isProject?sid:scope.projectId)?" · 1 个项目分身":""}`:"尚未加入")),h('div',{className:'eva-members-actions'},isProject&&canEdit&&manage&&h(Button,{theme:'light',type:'tertiary',onClick:()=>setRoleMember(actor)},'角色设置'),canEdit&&h(Button,{theme:'solid',type:'primary',icon:h(PlusIcon,{size:16}),onClick:()=>{setAddOpen(true);setError('');}},'添加成员'))),
