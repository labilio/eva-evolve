(function(root){
  ...
  root.EvaMembersUI={create({React:R,Button,Select,Modal,Table,Input,Tag,Checkbox,Radio,Switch,PlusIcon,CircleMinusIcon,CameraIcon,CloseIcon,BackIcon,SearchIcon,ProjectIcon,useNavigate},store,files){
    ...
    const identity=(item,selected=false)=>{if(item.kind==='project'){const tone=root.EvaProjectAppearance.css(item.project||{id:item.id,name:item.name});return h('span',{className:'eva-member-picker__project-identity'},h('span',{className:'eva-member-picker__project-icon','aria-hidden':true,style:{backgroundColor:tone.surface,color:tone.accent}},ProjectIcon?h(ProjectIcon,{size:16}):null),h('span',{className:'eva-member-picker__project-text'},h('span',{className:'eva-member-picker__project-name',title:item.name},item.name),selected?null:h('span',{className:'eva-member-picker__project-detail'},item.detail)));}if(item.kind==='channel'){const channel=item.channel||{id:item.id,personId:item.personId,identityAvatarUrl:item.identityAvatarUrl,color:item.color};const appearance=item.appearance||channel.identityAppearance;const avatar=appearance?root.EvaAIIdentity.avatar(appearance,28,h):h('img',{className:'eva-members-human-avatar',alt:'',src:root.EvaAvatar.conversationUri(channel),draggable:false});return item.ai?h('span',{className:'eva-members-ai-identity'},avatar,h('span',{className:'eva-identity-copy'},h('span',{className:'eva-identity-name-row'},h('span',{className:'eva-identity-name-text',title:item.name},item.name),root.EvaAIIdentity.badge(h)))):h('span',{className:'eva-members-human-identity is-compact'},avatar,h('span',{className:'eva-members-human-copy'},h('span',{className:'eva-members-human-name',title:item.name},item.name)));}return item.kind==='human'?h(HumanIdentity,{id:item.id,detail:selected?'':item.detail,compact:true}):h('span',{className:'eva-members-ai-identity'},root.EvaAIIdentity.avatar(item.appearance||root.EvaAIIdentity.cloneAppearance(store.person(item.ownerId)),28,h),h('span',{className:'eva-identity-copy'},h('span',{className:'eva-identity-name-row'},h('span',{className:'eva-identity-name-text',title:item.name},item.name),root.EvaAIIdentity.badge(h))));};
    function MemberPicker({visible,title,items,onCancel,onSubmit,single=false,allowEmpty=false,submit='确认添加',withName=false,nameField,memberLabel='成员',groups,initialSelectedIds=[],minimumSelection,search,searchLabel='搜索可选成员',searchPlaceholder='搜索可选成员',emptyTitle='暂无可选成员',emptyDescription='',noResultsText='没有匹配的成员',className='',getPopupContainer,zIndex,humanHint,itemNoun}){
      ...
      const [ids,setIds]=R.useState([]),[name,setName]=R.useState(''),[query,setQuery]=R.useState(''),[error,setError]=R.useState(''),[invalid,setInvalid]=R.useState(null);
      R.useEffect(()=>{if(!visible)return;setIds(initialSelectedIds.filter(id=>items.some(item=>item.id===id&&!item.disabled)));setName(field?.initialValue||'');setQuery('');setError('');setInvalid(null);},[visible,title,field?.initialValue,initialSelectedIds.join('|')]);
      ...
      const invalidReason=()=>{
        if(field?.required&&!name.trim())return {scope:'name',text:'请输入'+field.label};
        if(field?.maxLength&&name.trim().length>field.maxLength)return {scope:'name',text:field.label+'最多 '+field.maxLength+' 个字符'};
        if(chosen.length<required)return {scope:'members',text:'请至少选择 '+required+' 位'+memberLabel};
        return null;
      };
      ...
      return h(Modal,{className:('eva-members-modal eva-picker-modal eva-member-picker-modal '+className).trim(),width:680,title,visible,zIndex,onCancel,footer,getPopupContainer,maskClosable:true},
        h('div',{className:'eva-member-picker'+(field?'':' eva-member-picker--selection-only')},
          field&&h('div',{className:'eva-member-picker__field'},h('label',{htmlFor:field.id||'eva-member-picker-name'},field.label),h(Input,{id:field.id||'eva-member-picker-name','aria-label':field.label,value:name,onChange:value=>{setName(value);setError('');setInvalid(null);},placeholder:field.placeholder,maxLength:field.maxLength,autoFocus:field.autoFocus!==false,...(invalid&&invalid.scope==='name'?{'aria-invalid':true,'aria-describedby':'eva-member-picker-name-error'}:{})}),
            invalid&&invalid.scope==='name'&&h('p',{role:'alert',className:'eva-member-picker__field-error',id:'eva-member-picker-name-error'},invalid.text)),
          h('div',{className:'eva-member-picker__members'+(field?'':' eva-member-picker__members--selection-only')},
            ...
            h('div',{className:'eva-member-picker__panel',...(field?{'aria-labelledby':'eva-member-picker-members-label'}:{'aria-label':memberLabel})},
              ...
              h('aside',{className:'eva-member-picker__selected','aria-label':'已选'+selectionNoun},
                ...
                h('div',{className:'eva-member-picker__selected-list'},chosen.map(item=>h('div',{key:item.id,className:'eva-member-picker__selected-item'},h('span',{className:'eva-member-picker__selected-avatar'},identity(item,true)),h('span',{className:'eva-member-picker__selected-name',title:item.name},h('span',{className:'eva-member-picker__selected-name-text'},item.name),(item.kind==='channel'?item.ai:item.kind!=='human'&&item.kind!=='project')&&root.EvaAIIdentity.badge(h)),h(Button,{theme:'borderless',type:'tertiary',size:'small',icon:h(CloseIcon,{size:16}),'aria-label':'移除 '+item.name,onClick:()=>toggle(item.id)}))),!chosen.length&&h('p',null,'从左侧选择'+selectionNoun)))),
    ...
    function Members({scopeId}){
      ...
      return h('section',{className:'eva-members'},sid==='prod'&&new URLSearchParams(root.location.search).has('picker-preview')&&h(PickerPreview,{MemberPicker}),h('div',{className:'eva-members-toolbar'},h('div',null,h('h3',null,isProject?'项目成员':'群聊成员'),h('p',{className:'eva-members-muted'},joined?`${scope.humans.length} 位联系人 · ${scope.cloneIds.length} 个 AI 分身${store.projectAgent(isProject?sid:scope.projectId)?" · 1 个项目分身":""}`:"尚未加入")),h('div',{className:'eva-members-actions'},isProject&&canEdit&&manage&&h(Button,{theme:'light',type:'tertiary',onClick:()=>setRoleMember(actor)},'角色设置'),canEdit&&h(Button,{theme:'solid',type:'primary',icon:h(PlusIcon,{size:16}),onClick:()=>{setAddOpen(true);setError('');}},'添加成员'))),
        ...
        h(SinglePersonPicker,{key:'transfer'+sid+actor,visible:action?.type==='transfer',title:isProject?'转让项目负责人':'转让群主',submit:'确认转让',items:humanItems(humanRows.filter(p=>p.id!==actor),isProject?sid:scope.projectId),onCancel:()=>setAction(null),onSubmit:chosen=>{store.transfer(sid,actor,chosen[0].id);setAction(null);}}),
