(function(root){
  ...
  root.EvaPickerPreview={create({React:R,Button,Select,Modal,Input,Tag,Checkbox,Radio},store){
    ...
    function SelectionBody({items,selected,onChange,single,showProjectFilter,humanHint,humanLabel='人类成员',emptyTitle='暂无可选成员',emptyDescription='当前范围内没有可选择的人类或分身。'}){
      ...
      return h('div',{className:'eva-picker-body'},showProjectFilter&&h('div',{className:'eva-picker-filter'},h(Select,{'aria-label':'按项目筛选',value:project,onChange:setProject,optionList:[{value:'all',label:'全部可选人员'},{value:'供应链运营协同',label:'供应链运营协同'}]})),
        h('div',{className:'eva-picker-list'},
          !visible.length&&h('div',{className:'eva-picker-empty'},h('strong',null,emptyTitle),h('p',null,emptyDescription)),
          (single?[null]:['human','clone']).map(kind=>{const rows=kind?visible.filter(p=>p.kind===kind):visible;return rows.length>0&&h('section',{key:kind||'all'},!single&&h('div',{className:'eva-picker-group'},h('strong',null,kind==='human'?humanLabel:'我的 AI 分身'),h('span',null,rows.length),(()=>{const hint=kind==='clone'?'仅可带入自己的分身，确认后直接加入。':humanHint||humanLabel+'可多选，确认后直接加入。';return hint?h('small',null,hint):null;})()),rows.map(p=>h('label',{key:p.id,className:'eva-picker-row'+(selected.includes(p.id)?' is-selected':'')},h(single?Radio:Checkbox,{'aria-label':'选择'+p.name,name:'eva-picker-choice',checked:selected.includes(p.id),disabled:!!p.disabled,onChange:()=>{if(!p.disabled)toggle(p.id);}}),p.kind==='clone'?root.EvaAIIdentity.avatar(root.EvaAIIdentity.cloneAppearance(store.person(p.ownerId)),36,h):h('span',{className:'eva-picker-avatar'},h('img',{alt:'',src:root.EvaAvatar.personUri(p.id)})),h('span',{className:'eva-picker-person'},h('span',{className:'eva-picker-name'},h('span',{className:'eva-picker-name-text',title:p.name},p.name),p.kind==='clone'&&root.EvaAIIdentity.badge(h)),p.detail&&h('span',{className:'eva-picker-detail'},p.detail)),p.selectionHint&&h('span',{className:'eva-picker-selection-hint'},p.selectionHint))))})));
    }
