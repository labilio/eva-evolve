...
function EvaForwardMessagesDialog({request,channels,store,actorId,onClose,onSent,getContainer}) {
  ...
  const submit=()=>{try{const selected=unique.find(channel=>channel.id===target);if(!selected)throw new Error('请选择目标会话');if(selected.personId)store.openDirect(actorId,selected.personId);store.forwardMessages(request.sourceId,target,actorId,request.messages,request.mode);onSent();}catch(e){setError(e.message||'转发失败');}};
  return h('div',{ref:setHost,className:'eva-forward-host'},host&&h(Modal,{visible:true,title:'转发消息',width:520,getPopupContainer:()=>host,onCancel:onClose,footer:h('div',{className:'eva-forward-footer'},h(Button,{onClick:onClose},'取消'),h(Button,{theme:'solid',type:'primary',disabled:!target,onClick:submit},'确认转发'))},
    h('p',null,'选择目标会话，将'+(request.mode==='merge'?'合并':'逐条')+'转发 '+request.messages.length+' 条消息'),
...
function EvaAssistantEditor({request,host,onClose}) {
  ...
  const scope=reactExports.useRef(null), avatarInput=reactExports.useRef(null), alive=reactExports.useRef(true);
  ...
  function uploadAvatar(event){const input=event.currentTarget,file=input.files?.[0];input.value='';if(!file)return;
  ...
  const editor=h('section',{className:'eva-create-assistant-modal',ref:scope,'aria-label':editorTitle},
      h('header',{className:'eva-create-assistant-modal__head'},persona?h(EvaAIIdentityAvatar,{appearance,size:34}):h('div',{className:'eva-editor-avatar-upload'},
        h('input',{ref:avatarInput,type:'file',hidden:true,accept:'image/png,image/jpeg,image/webp','aria-label':'选择助理头像图片',onChange:uploadAvatar}),
        h('button',{type:'button',className:'eva-editor-avatar-button',title:'点击上传头像','aria-label':draft.avatar?'更换助理头像':'上传助理头像',disabled:busy,onClick:()=>avatarInput.current?.click()},h(EvaAIIdentityAvatar,{appearance,size:34}),h('span',{className:'eva-editor-avatar-action','aria-hidden':true},h(Plus$c,{size:12,strokeWidth:1.75})))),
...
function EvaAITeamPage() {
  ...
  function identitySessionsDisclosure(item,expanded,hasSessions){
    if(!hasSessions)return null;
    return h('button',{type:'button',className:'eva-ai-team__identity-sessions-more','aria-expanded':expanded,'aria-controls':'ai-sessions-'+item.id,'aria-label':(expanded?'收起 ':'展开查看 ')+item.name+' 会话',onClick:()=>setCollapsed(value=>({...value,[item.id]:expanded}))},
      h('span',null,expanded?'收起':'展开查看'),
      h(ChevronDown,{size:12,className:'eva-ai-team__identity-sessions-more-chevron'+(expanded?' is-expanded':''),'aria-hidden':true}));
  }
  function identityItem(i){
    ...
    const expanded=collapsed[i.id]===false, emptyPersona=i.role==='persona'&&sessions.length===0, hasUnread=sessions.some(item=>item.unreadCount>0), abnormal=i.status==='offline'||(i.role==='persona'&&i.syncStatus!=='synced'),assistant=i.role==='assistant';
    return h('section',{className:'eva-ai-team__identity',key:i.id},
      h('div',{className:'eva-ai-team__identity-heading',onContextMenu:event=>openIdentityMenu(event,i)},
        ...
        h('button',{type:'button',className:'eva-ai-team__identity-button'+(assistant?' has-config-avatar':''),'aria-expanded':expanded,'aria-controls':'ai-sessions-'+i.id,onKeyUp:event=>identityMenuKey(event,i),onClick:()=>{setCollapsed(value=>({...value,[i.id]:emptyPersona?false:expanded}));if(emptyPersona)choose(i.id,null);}},
      ...
      expanded&&h('div',{className:'eva-ai-team__sessions',id:'ai-sessions-'+i.id},
        !emptyPersona&&identity?.id===i.id&&!session&&h('button',{type:'button',className:'eva-ai-team__session is-selected','aria-current':'true',onClick:()=>choose(i.id,null)},h('span',{className:'eva-ai-team__session-title'},'新对话')),
        ...
        !emptyPersona&&sessions.length===0&&!(identity?.id===i.id&&!session)&&h('button',{type:'button',className:'eva-ai-team__session',onClick:()=>newConversation(i.id)},h(Plus$c,{size:16}),h('span',{className:'eva-ai-team__session-title'},'新建会话'))),
      identitySessionsDisclosure(i,expanded,sessions.length>0));
  }
  function employeeItem(item){
    const sessions=digitalStore.sessions(item.id), expanded=collapsed[item.id]===false, hasUnread=sessions.some(session=>session.unreadCount>0);
    return h('section',{className:'eva-ai-team__identity',key:item.id},
      h('div',{className:'eva-ai-team__identity-heading',onContextMenu:event=>openIdentityMenu(event,item,true)},
        h('button',{type:'button',className:'eva-ai-team__identity-button','aria-expanded':expanded,'aria-controls':'ai-sessions-'+item.id,onKeyUp:event=>identityMenuKey(event,item,true),onClick:()=>setCollapsed(value=>({...value,[item.id]:expanded}))},
      ...
      identitySessionsDisclosure(item,expanded,sessions.length>0));
  }
    ...
    return h('section',{className:'eva-ai-team__team',key:group.id},
      h('div',{className:'eva-ai-team__team-heading'+(selected&&!selection.sessionId?' is-selected':'')},
        h('button',{type:'button',className:'eva-ai-team__team-button','aria-label':'进入团队会话 '+group.name,'aria-current':selected&&!selection.sessionId?'true':undefined,'aria-expanded':expanded,'aria-controls':threadsId,onClick:()=>{if(!expanded)setCollapsed(value=>({...value,[group.id]:false}));choose(group.id,null);}},
          h('img',{className:'eva-ai-team__team-avatar',src:window.EvaAvatar.uri({kind:'group',id:group.id}),alt:''}),
  ...
  return h('div',{className:'eva-ai-team'},
    ...
    source=root.__evaCut(source,'EvaIMComposer=({placeholder:rt,onSend:ct,initialDraft:evaInitialDraft="",onDraftChange:evaDraftChange,disabled:evaDisabled=false})=>{const[ut,pt]=reactExports.useState(evaInitialDraft)',
      'EvaIMComposer=({placeholder:rt,onSend:ct,initialDraft:evaInitialDraft="",onDraftChange:evaDraftChange,disabled:evaDisabled=false,scopeId:evaMentionScope})=>{const[evaMentionOpen,setEvaMentionOpen]=reactExports.useState(false),[evaMentionQuery,setEvaMentionQuery]=reactExports.useState(""),[evaMentionIndex,setEvaMentionIndex]=reactExports.useState(0),evaMentionRange=reactExports.useRef(null);const[ut,pt]=reactExports.useState(evaInitialDraft)','共享输入区提及状态');
    ...
    cut('searchPeople(ci).filter(Zi=>Zi.uid!==ME&&!Zi.ai).slice(0,6)',
      'evaMemberStore.people().filter(p=>p.id!==evaActorId&&p.name.includes(ci)).slice(0,6).map(p=>({...p,uid:p.id}))', '搜索仅展示当前有效人类身份');
  ...
  for(const continuation of ['!1','ro'])cut('...rowProps(ci,'+continuation+'),onContextMenu:', '...rowProps(ci,'+continuation+'),onAvatarClick:()=>setEvaIdentityProfile(ci.sender),onSenderNameClick:()=>setEvaIdentityProfile(ci.sender),onContextMenu:', '消息身份点击 '+continuation);
    ...
    cut('React.createElement("span",{className:"t"},Sa.name))),ut&&!fa&&(ct?.scopeNameOf[Sa.id]??Kr[Sa.id])&&React.createElement("span",{className:"ch-head__scope"},ct?.scopeNameOf[Sa.id]??Kr[Sa.id]),',
      'React.createElement("span",{className:"t"},Sa.name),!ct?.conversationOnly&&evaMemberStore.conversationContext(fa?.id||Sa.id,evaActorId)&&React.createElement("span",{className:"eva-chat-project-context",title:evaMemberStore.conversationContext(fa?.id||Sa.id,evaActorId).projectName},React.createElement(LayoutGrid,{size:12,style:{color:window.EvaProjectAppearance.css(evaMemberStore.conversationContext(fa?.id||Sa.id,evaActorId)).accent},"aria-hidden":true}),React.createElement("span",null,evaMemberStore.conversationContext(fa?.id||Sa.id,evaActorId).projectName)))),','聊天顶部项目归属');
