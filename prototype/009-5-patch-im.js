...
function EvaAITeamPage() {
  ...
  function conversationMenu(identityId,record,employee=false){
    ...
    return h(Dropdown,{trigger:'click',position:'bottomRight',clickToHide:true,getPopupContainer:()=>rail.current,
      ...
      h('span',{className:'eva-ai-team__menu-anchor'},h(TooltipComponent,{content:'会话操作',position:'top'},h(Button,{theme:'borderless',type:'tertiary',size:'small',icon:h(EllipsisIcon),'aria-label':'会话操作 '+record.title}))));
  }
  function teamThreadMenu(group,record){
    return h(Dropdown,{trigger:'click',position:'bottomRight',clickToHide:true,getPopupContainer:()=>rail.current,
      ...
      h('span',{className:'eva-ai-team__menu-anchor'},h(TooltipComponent,{content:'会话操作',position:'top'},h(Button,{theme:'borderless',type:'tertiary',size:'small',icon:h(EllipsisIcon),'aria-label':'会话操作 '+record.name}))));
  }
  ...
  function teamGroupItem(group){
    ...
    return h('section',{className:'eva-ai-team__team',key:group.id},
      h('div',{className:'eva-ai-team__team-heading'+(selected&&!selection.sessionId?' is-selected':'')},
        h('button',{type:'button',className:'eva-ai-team__team-button','aria-label':'进入 AI 小队会话 '+group.name,'aria-current':selected&&!selection.sessionId?'true':undefined,'aria-expanded':expanded,'aria-controls':threadsId,onClick:()=>{if(!expanded)setCollapsed(value=>({...value,[group.id]:false}));choose(group.id,null);}},
  ...
  return h('div',{className:'eva-ai-team'},
    h('aside',{className:'eva-ai-team__sidebar','aria-label':'我的 AI',ref:rail},
      ...
      h('header',{className:'eva-ai-team__sidebar-header eva-rail-header'},h('h1',null,'我的 Agent'),h(Dropdown,{trigger:'click',position:'bottomRight',clickToHide:true,getPopupContainer:()=>rail.current,render:h(Dropdown.Menu,null,
        ...
        h(Dropdown.Item,{icon:h(Sparkles,{size:16}),onClick:openPersonalAssistant},'新建个人助理'))},h('span',{onFocus:event=>{groupEditorOpener.current=event.target;}},h(TooltipComponent,{content:'新建',position:'bottom'},h(Button,{className:'eva-ai-team__create',theme:'borderless',type:'tertiary',icon:h(Plus$c,{size:18,strokeWidth:1.75}),'aria-label':'新建'}))))),
    ...
    source=root.__evaCut(source,'Ta=reactExports.useMemo(()=>[...ca,...oa[va]??[]],[ca,va,oa])',
      'evaAllMessages=reactExports.useMemo(()=>[...ca,...evaMemberStore.messagesFor(va,evaActorId),...oa[va]??[]].sort((a,b)=>Number(!!b.fixtureId?.startsWith("project-agent-welcome:"))-Number(!!a.fixtureId?.startsWith("project-agent-welcome:"))),[ca,va,oa,evaActorId,evaMemberRevision]),Ta=evaMemberStore.visibleMessages(va,evaActorId,evaAllMessages),evaShowAiFilter=!!evaTeamGlobal,evaHideAi=evaShowAiFilter&&evaMemberStore.chatPreferences(va,evaActorId).hideAi===true,evaStreamFilter=(()=>{if(!evaHideAi)return null;const evaKept=Ta.filter(evaM=>evaM.kind==="divider"||!(evaM.sender?.ai===true||evaM.sender?.identityAppearance)),evaStream=evaKept.filter((evaM,evaI)=>evaM.kind!=="divider"||(evaI<evaKept.length-1&&evaKept[evaI+1].kind!=="divider")),evaIndex=new Map();let evaPos=0;Ta.forEach((evaM,evaI)=>{if(evaStream.indexOf(evaM)>=0)evaIndex.set(evaI,evaPos++)});return{stream:evaStream,index:evaIndex}})(),evaStreamMessages=evaStreamFilter?evaStreamFilter.stream:Ta','消息记录持久化读取与 AI 发言过滤');
  ...
  for(const continuation of ['!1','ro'])cut('...rowProps(ci,'+continuation+'),onContextMenu:', '...rowProps(ci,'+continuation+'),onAvatarClick:()=>setEvaIdentityProfile(ci.sender),onSenderNameClick:()=>setEvaIdentityProfile(ci.sender),onContextMenu:', '消息身份点击 '+continuation);
    ...
    cut('React.createElement("span",{className:"ops"},evaCanOpenProjectTasks&&',
      'React.createElement("span",{className:"ops"},!ct?.conversationOnly&&!(evaMembershipProjectId&&evaMembershipProjectId===evaTaskProjectId)&&evaMemberStore.conversationContext(fa?.id||Sa.id,evaActorId)&&React.createElement(TooltipComponent,{content:"进入项目 "+evaMemberStore.conversationContext(fa?.id||Sa.id,evaActorId).projectName,position:"top"},React.createElement("button",{type:"button",className:"eva-chat-project-jump","aria-label":"进入项目 "+evaMemberStore.conversationContext(fa?.id||Sa.id,evaActorId).projectName,onClick:()=>{const evaHeaderProject=evaMemberStore.conversationContext(fa?.id||Sa.id,evaActorId);if(evaHeaderProject)window.dispatchEvent(new CustomEvent("eva:open-inline-project",{detail:{projectId:evaHeaderProject.projectId}}))}},React.createElement(LayoutGrid,{size:14,className:"eva-chat-project-jump__icon",style:{color:window.EvaProjectAppearance.css(evaMemberStore.conversationContext(fa?.id||Sa.id,evaActorId)).accent},"aria-hidden":true}),React.createElement("span",{className:"eva-chat-project-jump__name"},evaMemberStore.conversationContext(fa?.id||Sa.id,evaActorId).projectName))),!ct?.conversationOnly&&!(evaMembershipProjectId&&evaMembershipProjectId===evaTaskProjectId)&&evaMemberStore.conversationContext(fa?.id||Sa.id,evaActorId)&&React.createElement("span",{className:"eva-chat-head-divider","aria-hidden":true}),evaShowAiFilter&&React.createElement(TooltipComponent,{content:evaHideAi?"显示 AI 回复":"隐藏 AI 回复",position:"top"},React.createElement("button",{type:"button",className:"op eva-chat-ai-filter"+(evaHideAi?" is-off":""),"aria-label":evaHideAi?"显示 AI 回复":"隐藏 AI 回复","aria-pressed":evaHideAi,onClick:()=>{evaMemberStore.setChatPreferences(va,evaActorId,{hideAi:!evaHideAi});evaSelection!==null&&setEvaSelection(null)}},React.createElement("span",{className:"eva-chat-ai-filter__icon","aria-hidden":true},window.EvaAIIdentity.badge(React.createElement),evaHideAi&&React.createElement("span",{className:"eva-chat-ai-filter__slash"})))),React.createElement(TooltipComponent,{content:"查找聊天内容",position:"top"},React.createElement("button",{type:"button",className:`op eva-chat-search-entry${Mt==="search"?" is-on":""}`,"aria-label":"查找聊天内容","aria-expanded":Mt==="search","aria-controls":"eva-conversation-search-panel",onClick:()=>Dt(mode=>mode==="search"?"none":"search")},React.createElement(Search$1,{size:20,color:"currentColor"}))),evaCanOpenProjectTasks&&', '统一 IM 查找入口与顶部项目跳转');
    cut('React.createElement("span",{className:`op${Mt==="threads"?" is-on":""}`,role:"button",tabIndex:0,title:"子区",onClick:()=>Dt(ci=>ci==="threads"?"none":"threads"),onKeyDown:ci=>{ci.key==="Enter"&&Dt(Zi=>Zi==="threads"?"none":"threads")}},React.createElement(ThreadIcon,{size:20,color:"currentColor"}))',
      'React.createElement(TooltipComponent,{content:"子区",position:"top"},React.createElement("span",{className:`op${Mt==="threads"?" is-on":""}`,role:"button",tabIndex:0,"aria-label":"子区",onClick:()=>Dt(ci=>ci==="threads"?"none":"threads"),onKeyDown:ci=>{ci.key==="Enter"&&Dt(Zi=>Zi==="threads"?"none":"threads")}},React.createElement(ThreadIcon,{size:20,color:"currentColor"})))', 'IM 子区按钮 tooltip');
    cut('React.createElement("span",{className:`op${Mt==="info"?" is-on":""}`,role:"button",tabIndex:0,title:fa?"子区信息":"聊天信息",onClick:()=>Dt(ci=>ci==="info"?"none":"info"),onKeyDown:ci=>{ci.key==="Enter"&&Dt(Zi=>Zi==="info"?"none":"info")}},React.createElement(EllipsisIcon,null))',
      'React.createElement(TooltipComponent,{content:fa?"子区信息":"聊天信息",position:"top"},React.createElement("span",{className:`op${Mt==="info"?" is-on":""}`,role:"button",tabIndex:0,"aria-label":fa?"打开子区信息":"打开聊天信息",onClick:()=>Dt(ci=>ci==="info"?"none":"info"),onKeyDown:ci=>{ci.key==="Enter"&&Dt(Zi=>Zi==="info"?"none":"info")}},React.createElement(EllipsisIcon,null)))', 'IM 聊天信息按钮 tooltip');
