...
`;
    ...
    source = root.__evaCut(source, 'getSizingData=rt=>{const ct=window.getComputedStyle(rt);', 'getSizingData=rt=>{if(!rt||!rt.isConnected)return null;const ct=window.getComputedStyle(rt);', 'ignore detached textarea resize');
        ...
        var replacements = [
          ...
          ['function AssigneePicker(', 'function EvaLoopIdentityName({person}){const h=React.createElement,store=evaMembers().store,id=person?.id,human=id&&store.person(id),ai=!human&&person?.type!=="member"&&person?.type!=="squad"&&(person?.type==="agent"||!!store.clone(id)||!!store.employee(id)||String(id||"").startsWith("project-agent:")||scoped(BY_SPACE.agents).some(item=>item.id===id));return h("span",{className:"eva-loop-identity-name"},h("span",{className:"eva-loop-identity-name-text"},person?.name||""),ai&&window.EvaAIIdentity.badge(h));}function evaTaskPopupContainer(){const evaTitlebar=document.querySelector(".app-titlebar,.topbar,.eva-topbar"),evaTop=evaTitlebar?evaTitlebar.getBoundingClientRect().bottom:0;let evaHost=document.querySelector(".eva-task-popup-host");return evaHost||(evaHost=document.createElement("div"),evaHost.className="eva-task-popup-host",document.body.appendChild(evaHost)),evaHost.style.top=evaTop+"px",evaHost}function AssigneePicker('],
    ...
    source=root.__evaCut(source,
      ...
      'onVisibleChange:Vt=>{evaSetOpen(Vt);if(Vt){Pt(!0);evaSetQ("");evaInputRef.current&&(evaInputRef.current.value="");evaSetExp({});evaSetActive(-1);const evaTriggerRect=evaTriggerRef.current?evaTriggerRef.current.getBoundingClientRect():null,evaTopbarRects=[...document.querySelectorAll(".app-titlebar,.topbar,.eva-topbar")],evaTopLimit=evaTopbarRects.reduce((evaMax,evaEl)=>Math.max(evaMax,evaEl.getBoundingClientRect().bottom),0),evaGap=8,evaChrome=St.length>5?70:16,evaSpaceBelow=evaTriggerRect?Math.max(0,window.innerHeight-evaTriggerRect.bottom-evaGap):480,evaSpaceAbove=evaTriggerRect?Math.max(0,evaTriggerRect.top-evaTopLimit-evaGap):480;evaSetMaxH(Math.max(96,Math.min(480,Math.max(evaSpaceBelow,evaSpaceAbove)-evaChrome)));setTimeout(()=>{evaInputRef.current&&evaInputRef.current.focus()},0)}else{evaSetMaxH(null)}}',
      '面板打开时重置搜索与展开状态，并按可用空间自适应菜单上限');
    // 选人触发元素 ref：打开时用它的位置计算「标题栏下方 / 视口底」的可用空间，
    // 让菜单上限取 min(480px, 可用空间)，避免长列表把弹层顶到系统标题栏。
    source=root.__evaCut(source,
      'React.createElement("span",{className:"loop-assignee-trigger",style:{fontSize:pt==="small"?12:13}},',
      'React.createElement("span",{ref:evaTriggerRef,className:"loop-assignee-trigger",style:{fontSize:pt==="small"?12:13}},',
      '选人触发器 ref 用于自适应限高');
    // 弹层宿主固定在系统标题栏下沿（.eva-task-popup-host），Semi 的溢出回弹
    // 只在该区域内进行，长列表不会再被顶到标题栏上方；配合 onVisibleChange 的自适应限高。
    source=root.__evaCut(source,
      'render:Qt,trigger:"click",position:"bottomLeft"',
      'render:Qt,trigger:"click",position:"bottomLeft",visible:evaOpen,getPopupContainer:evaTaskPopupContainer',
      '选人弹层限制在标题栏下方内容区');
