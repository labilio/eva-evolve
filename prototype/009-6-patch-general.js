...
`;
    ...
    source = root.__evaCut(source, 'getSizingData=rt=>{const ct=window.getComputedStyle(rt);', 'getSizingData=rt=>{if(!rt||!rt.isConnected)return null;const ct=window.getComputedStyle(rt);', 'ignore detached textarea resize');
        ...
        function evaLoopMentionSuggestion(){
          ...
          function draw(){if(!root||!state)return;root.render(React.createElement(evaMembers().ui.MentionPicker,{visible:true,groups:groupedItems(),query:state.query,activeIndex:state.index,onActiveChange:index=>{state.index=index;draw()},onClose:close,onChoose:(name,id,item)=>commit(name,id,item)}));place()}
