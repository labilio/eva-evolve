...
SiderFooter=props=>{const[visible,setVisible]=reactExports.useState(false),navigate=useNavigate(),store=evaMembers().store;reactExports.useSyncExternalStore(store.subscribe,store.getSnapshot,store.getSnapshot);const actorId=store.snapshot().actorId,person=store.person(actorId);return React.createElement(React.Fragment,null,React.createElement("footer",{className:classNames("eva-sider-footer",props.collapsed&&"is-collapsed")},React.createElement("div",{className:"eva-sider-account","aria-label":"当前身份："+(person?.name||actorId)},React.createElement("img",{className:"eva-sider-account__avatar",src:window.EvaAvatar.personUri(actorId),alt:""}),React.createElement("span",{className:"eva-sider-account__name",title:person?.name||actorId},person?.name||actorId)),React.createElement(Button,{className:"eva-settings-trigger",theme:"borderless",type:"tertiary","aria-label":"设置",title:"设置",onClick:()=>setVisible(true)},React.createElement(Settings,{size:18}))),window.EvaSettingsUI.render({visible,onClose:()=>setVisible(false),navigate},{React:reactExports,Modal,Button,icons:{ChevronDown,MessageSquare,general:Settings,appearance:Monitor,engine:SlidersHorizontal,im:Bot,browser:Globe,mail:Mail$1,memory:Brain$8,shortcuts:Keyboard,about:Info$4,usage:createLucideIcon("chart-column",[["path",{d:"M3 3v18h18",key:"axes"}],["path",{d:"M18 17V9",key:"bar1"}],["path",{d:"M13 17V5",key:"bar2"}],["path",{d:"M8 17v-3",key:"bar3"}]])},pages:{general:EvaGeneral,appearance:EvaAppearance,engine:EvaEngine,im:EvaImBot,browser:EvaBrowser,mail:EvaMail,memory:EvaMemory,shortcuts:EvaShortcut,about:EvaAbout}}))},
...
`;
        siderArchitecture = siderArchitecture
          ...
          .replace('React.createElement(SiderMessagesEntry,{key:gt,...ct,isActive:rt.activeNavId===gt,onClick:()=>rt.navigate("/messages")})', 'React.createElement(SiderEvaStub,{key:gt,label:"我的消息",icon:React.createElement(MessageSquare,{size:16,strokeWidth:1.8,className:"block leading-none"}),...ct,isActive:rt.activeNavId===gt,onClick:()=>rt.navigate("/messages")})')
