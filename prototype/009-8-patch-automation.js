...
`;
  ...
  const Nt=async(Dt,Ft)=>{try{await updateAutopilot(Dt.id,{status:Ft?"active":"paused"}),xt()}catch(Qt){Toast.error(Qt?.message??rt("loop.toast.saveFailed"))}};
  const Mt=async Dt=>{try{await triggerAutopilot(Dt.id),Toast.success(rt("loop.automation.runStarted"))}catch(Ft){Toast.error(Ft?.message??rt("loop.automation.runFailed"))}};
  const Dt=async Ft=>{try{await deleteAutopilot(Ft),Toast.success(rt("loop.toast.deleted")),xt()}catch(Qt){Toast.error(Qt?.message??rt("loop.toast.deleteFailed"))}};
  const Ft=Qt=>confirmDelete({title:rt("loop.automation.confirmDelete"),content:Qt.title,okText:rt("loop.action.delete"),cancelText:rt("loop.action.cancel"),onOk:()=>Dt(Qt.id)});
  const Qt=Vt=>{const Ht=Vt.status==="paused",jt=Vt.last_run_status?AUTOPILOT_RUN_DOT[Vt.last_run_status]??AUTOPILOT_RUN_DOT_FALLBACK:AUTOPILOT_RUN_DOT_FALLBACK,tn=formatNextRunAt(Vt.next_run_at),Kt=Vt.assignee_name??Vt.assignee_id,Yt=Vt.assignee_id?.startsWith("project-agent:")||evaMembers().store.clone(Vt.assignee_id)||evaMembers().store.employee(Vt.assignee_id);return React.createElement("div",{key:Vt.id,className:"loop-automation-card",role:"listitem",tabIndex:0,onClick:()=>Pt(Vt),onKeyDown:Xt=>{if(Xt.key==="Enter"||Xt.key===" "){Xt.preventDefault();Pt(Vt)}}},
    React.createElement("div",{className:"loop-automation-card__head"},React.createElement("strong",{className:"loop-automation-card__title"},Vt.title),React.createElement("div",{onClick:Xt=>Xt.stopPropagation(),style:{display:"inline-flex"}},React.createElement(Switch,{checked:!Ht,size:"small",onChange:Xt=>Nt(Vt,Xt)}))),
    Vt.description&&React.createElement("div",{className:"loop-automation-card__desc"},Vt.description),
    React.createElement("div",{className:"loop-automation-card__meta"},tn&&React.createElement("span",null,tn),Ht&&React.createElement("span",{className:"loop-automation-card__paused"},rt("loop.automation.paused"))),
    React.createElement("div",{className:"loop-automation-card__foot"},React.createElement("span",{className:"loop-automation-card__assignee"},React.createElement(EvaLoopIdentityAvatar,{person:{id:Vt.assignee_id,name:Kt},size:24}),React.createElement("span",{className:"eva-identity-name-row"},React.createElement("span",{className:"loop-automation-card__assignee-name"},Kt),Yt&&window.EvaAIIdentity.badge(React.createElement))),React.createElement("span",{className:"loop-automation-card__runs"},React.createElement("i",{className:"loop-automation-card__dot",style:{background:jt}}))),
    React.createElement("div",{className:"loop-automation-card__more",onClick:Xt=>Xt.stopPropagation()},React.createElement(Dropdown,{trigger:"click",position:"bottomRight",clickToHide:true,render:React.createElement(Dropdown.Menu,null,React.createElement(Dropdown.Item,{icon:React.createElement(Play,{size:13}),onClick:()=>Mt(Vt)},rt("loop.automation.runNow")),React.createElement(Dropdown.Item,{icon:React.createElement(Trash2,{size:13}),type:"danger",onClick:()=>Ft(Vt)},rt("loop.action.delete")))},React.createElement(Button,{theme:"borderless",size:"small","aria-label":"更多操作",icon:React.createElement(Ellipsis,{size:16})}))));};
  return React.createElement("div",{className:"loop-page eva-project-automation"},React.createElement("div",{className:"loop-page__head"},React.createElement("div",{className:"loop-page__spacer"}),React.createElement(LoopButton,{icon:React.createElement(Plus$c,{size:14}),onClick:()=>St(true)},rt("loop.automation.create"))),React.createElement("div",{className:"loop-page__body"},pt?React.createElement("div",{className:"loop-page__center"},React.createElement(Spin,null)):ct.length===0?React.createElement("div",{className:"loop-empty"},React.createElement(Zap,{size:40,className:"loop-empty__icon"}),React.createElement("div",{className:"loop-empty__title"},rt("loop.automation.emptyTitle")),React.createElement("div",{className:"loop-empty__desc"},rt("loop.automation.emptyDesc")),React.createElement(LoopButton,{icon:React.createElement(Plus$c,{size:14}),onClick:()=>St(true),style:{marginTop:12}},rt("loop.automation.create"))):React.createElement("div",{className:"loop-automation-cards",role:"list"},ct.map(Qt))),React.createElement(CreateAutomationModal,{visible:gt,onClose:()=>St(false),onSaved:xt}));
