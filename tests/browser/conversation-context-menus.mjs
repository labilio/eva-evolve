...
test('conversation menus: real actions, object boundaries, drafts, keyboard and submenu',async()=>{
 ...
 try{
  ...
  const personal=page.locator('.eva-follow-channel > .wk-conv-compact-item').filter({hasText:'产品共创交流群'}).first();await personal.click({button:'right'});await menu.getByRole('menuitem',{name:'移动到分组',exact:true}).hover();await menu.getByRole('menuitem',{name:'新建分组',exact:true}).click();await page.getByRole('textbox',{name:'分组名称'}).fill('菜单测试分组');await page.getByRole('dialog',{name:'创建分组'}).getByRole('button',{name:'创建',exact:true}).click();await page.locator('.eva-follow-category').filter({hasText:'菜单测试分组'}).first().waitFor();
