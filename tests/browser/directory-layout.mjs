...
test('项目、通讯录与员工市场：实际列表布局、筛选及窄窗口操作', async()=>{
 ...
 try {
  ...
  assert.equal(await page.locator('.eva-digital-center__search input').count(),1,'市场卡片头部保留统一搜索框');
  ...
  await page.getByRole('button',{name:/^研发域 /}).click();
  const domains=await page.locator('.semi-table-tbody tr td:nth-child(3)').allTextContents();
  assert.ok(domains.length>0&&domains.every(d=>d==='研发域'),'业务域筛选仍生效');
  await page.getByRole('button',{name:/^全部业务域 /}).click();
  await page.locator('.eva-digital-center__search').click();
  const searchMenu=page.locator('.eva-digital-center__search-menu');
  await page.locator('.eva-digital-center__search input').fill('供应链');
  await searchMenu.locator('.semi-dropdown-item:has(.eva-digital-center__search-domain)',{hasText:'供应链'}).click();
  await searchMenu.waitFor({state:'hidden'});
  assert.match(await page.locator('.eva-digital-center__domain-filters .semi-button[aria-pressed="true"]').innerText(),/^供应链 /,'低频业务域经统一搜索选中后回显筛选行');
  const searched=await page.locator('.semi-table-tbody tr td:nth-child(3)').allTextContents();
  assert.ok(searched.length>0&&searched.every(d=>d==='供应链'),'统一搜索选中业务域后表格按域过滤');
