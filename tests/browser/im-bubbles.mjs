...
test('共享 IM 气泡：方向、Markdown、消息操作、草稿及入口往返', async()=>{
 ...
 await page.getByRole('region',{name:'消息多选'}).getByRole('button',{name:'逐条转发',exact:true}).click();await page.locator('.eva-fp-modal').waitFor();
 await page.keyboard.press('Escape');await page.locator('.eva-fp-modal').waitFor({state:'detached'});assert.ok(await page.locator('.wk-msg-row--selection-mode').count()>0,'Esc 先关闭转发弹窗，保留底层多选');
 ...
 await page.getByRole('button',{name:'取消',exact:true}).click();assert.equal(await page.locator('.eva-fp-modal').count(),0);
 await replied.click({button:'right'});await page.locator('.wk-contextmenus-open').getByText('转发',{exact:true}).click();
 await page.locator('.eva-forward-search input').fill('日常聊天');await page.locator('.eva-fp-row').filter({has:page.locator('.eva-fp-name-text',{hasText:'01 日常聊天与连续消息'})}).first().locator('input[type="checkbox"]').check();
 ...
 await editor.fill('未发送的草稿');await page.locator('.wk-chat-conversation-header-channel-avatar.is-settings-trigger').click();assert.equal(await editor.innerText(),'未发送的草稿');await page.locator('.eva-chat-settings-head button').first().click();
 ...
 await page.locator('.eva-forward-search input').fill('日常聊天');await page.locator('.eva-fp-row').filter({has:page.locator('.eva-fp-name-text',{hasText:'01 日常聊天与连续消息'})}).first().locator('input[type="checkbox"]').check();assert.ok(await page.locator('.ch-main').evaluate(e=>e.getBoundingClientRect().width)>600,'转发弹窗不挤压聊天区');await page.screenshot({path:'/tmp/eva-bubble-forward-dialog.png'});
 await page.locator('.eva-fp-modal').getByRole('button',{name:'发送',exact:true}).click();await page.locator('.eva-fp-modal').waitFor({state:'detached'});
 ...
 const conversationWidthBeforePreview=await page.locator('.ch-main__col').evaluate(element=>element.getBoundingClientRect().width);
 await page.locator('.wk-message-file').first().click();await page.locator('.wk-file-preview-panel').waitFor();await page.locator('.wk-file-preview-content').getByText('A-2409', {exact:false}).first().waitFor();
 const filePreviewGeometry=await page.locator('.ch-main').evaluate(main=>{const content=main.querySelector('.ch-main__col'),preview=main.querySelector('.ch-right-panel--file-preview'),contentRect=content.getBoundingClientRect(),previewRect=preview.getBoundingClientRect(),mainRect=main.getBoundingClientRect();return{contentWidth:contentRect.width,contentRight:contentRect.right,previewLeft:previewRect.left,previewRight:previewRect.right,mainRight:mainRect.right,position:getComputedStyle(preview).position};});
 assert.equal(filePreviewGeometry.position,'relative','文件预览是参与布局的并排右栏');assert.ok(filePreviewGeometry.contentWidth<conversationWidthBeforePreview-100,'打开预览会挤压消息内容');assert.ok(filePreviewGeometry.previewLeft>=filePreviewGeometry.contentRight-1,'预览位于消息内容右侧');assert.ok(Math.abs(filePreviewGeometry.previewRight-filePreviewGeometry.mainRight)<=1,'预览贴齐消息区域右边');
 const messagePreview=page.locator('.ch-right-panel--file-preview'),messageResize=messagePreview.getByRole('separator',{name:'调整文件预览宽度'}),messagePreviewBefore=await messagePreview.evaluate(element=>element.getBoundingClientRect().width),messageContentBefore=await page.locator('.ch-main__col').evaluate(element=>element.getBoundingClientRect().width),messageResizeBox=await messageResize.boundingBox();assert.ok(messageResizeBox);await page.mouse.move(messageResizeBox.x+messageResizeBox.width/2,messageResizeBox.y+80);await page.mouse.down();await page.mouse.move(messageResizeBox.x+messageResizeBox.width/2+64,messageResizeBox.y+80,{steps:4});await page.mouse.up();const messagePreviewAfter=await messagePreview.evaluate(element=>element.getBoundingClientRect().width),messageContentAfter=await page.locator('.ch-main__col').evaluate(element=>element.getBoundingClientRect().width);assert.ok(messagePreviewAfter<messagePreviewBefore-40,'消息文件预览支持向右拖拽缩小');assert.ok(messageContentAfter>messageContentBefore+40,'消息主内容获得释放的宽度');assert.equal(Number(await messageResize.getAttribute('aria-valuenow')),Math.round(messagePreviewAfter));
 await page.screenshot({path:'/tmp/eva-bubble-file-preview.png'});await page.keyboard.press('Escape');await page.locator('.wk-file-preview-panel').waitFor({state:'detached'});
 ...
 await page.locator('.eva-forward-search input').fill('日常聊天');await page.locator('.eva-fp-row').filter({has:page.locator('.eva-fp-name-text',{hasText:'01 日常聊天与连续消息'})}).first().locator('input[type="checkbox"]').check();await page.locator('.eva-fp-modal').getByRole('button',{name:'发送',exact:true}).click();await page.locator('.eva-fp-modal').waitFor({state:'detached'});
 ...
 await page.locator('.eva-fp-row').filter({has:page.locator('.eva-fp-name-text',{hasText:aiTarget.name.split(' / ')[1]})}).first().locator('input[type="checkbox"]').check();await page.locator('.eva-fp-modal').getByRole('button',{name:'发送',exact:true}).click();await page.locator('.eva-fp-modal').waitFor({state:'detached'});
 ...
 await aiForwarded.click({button:'right'});await page.locator('.wk-contextmenus-open').getByText('转发',{exact:true}).click();await page.locator('.eva-fp-row').filter({has:page.locator('.eva-fp-name-text',{hasText:employeeTarget.name.split(' / ')[1]})}).first().locator('input[type="checkbox"]').check();await page.locator('.eva-fp-modal').getByRole('button',{name:'发送',exact:true}).click();await page.locator('.eva-fp-modal').waitFor({state:'detached'});
 ...
 await aiForwarded.click({button:'right'});await page.locator('.wk-contextmenus-open').getByText('转发',{exact:true}).click();await page.locator('.eva-fp-row',{hasText:teamTarget.name}).first().locator('input[type="checkbox"]').check();await page.locator('.eva-fp-modal').getByRole('button',{name:'发送',exact:true}).click();await page.locator('.eva-fp-modal').waitFor({state:'detached'});
 await page.goto(origin+'/#/messages?evaIM=my-ai&evaIdentity='+encodeURIComponent(teamTarget.id));await page.getByRole('button',{name:'进入 AI 小队会话 '+teamTarget.name,exact:true}).click();await page.locator('.eva-im-bubble-row').getByText('数字员工引用回归',{exact:true}).waitFor();
