...
test('共享 IM 气泡：方向、Markdown、消息操作、草稿及入口往返', async()=>{
 ...
 await page.getByRole('button',{name:'退出多选',exact:true}).click();assert.equal(await page.locator('.wk-msg-row--selection-mode').count(),0);
 ...
 await page.getByRole('region',{name:'消息多选'}).getByRole('button',{name:'转发',exact:true}).click();
