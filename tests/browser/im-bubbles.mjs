...
test('共享 IM 气泡：方向、Markdown、消息操作、草稿及入口往返', async()=>{
 ...
 await editor.fill('未发送的草稿');await page.getByRole('button',{name:'打开聊天信息',exact:true}).click();assert.equal(await editor.innerText(),'未发送的草稿');await page.locator('.eva-chat-settings-head button').first().click();
