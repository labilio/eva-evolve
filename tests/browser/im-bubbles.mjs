...
test('共享 IM 气泡：方向、Markdown、消息操作、草稿及入口往返', async()=>{
 ...
 await replied.click({button:'right'});await page.locator('.wk-contextmenus-open').getByText('转发',{exact:true}).click();
 await page.locator('.eva-fp-row',{hasText:'01 日常聊天与连续消息'}).first().locator('input[type="checkbox"]').check();
 ...
 await page.locator('.eva-fp-row',{hasText:'01 日常聊天与连续消息'}).first().locator('input[type="checkbox"]').check();assert.ok(await page.locator('.ch-main').evaluate(e=>e.getBoundingClientRect().width)>600,'转发弹窗不挤压聊天区');await page.screenshot({path:'/tmp/eva-bubble-forward-dialog.png'});
 await page.locator('.eva-fp-modal').getByRole('button',{name:'发送',exact:true}).click();await page.locator('.eva-fp-modal').waitFor({state:'detached'});
 ...
 await page.locator('.eva-fp-row',{hasText:'01 日常聊天与连续消息'}).first().locator('input[type="checkbox"]').check();await page.locator('.eva-fp-modal').getByRole('button',{name:'发送',exact:true}).click();await page.locator('.eva-fp-modal').waitFor({state:'detached'});
 ...
 await aiForwarded.click({button:'right'});await page.locator('.wk-contextmenus-open').getByText('转发',{exact:true}).click();await page.getByRole('tab',{name:'我的 Agent',exact:true}).click();await page.locator('.eva-fp-row',{hasText:teamTarget.name}).first().locator('input[type="checkbox"]').check();await page.locator('.eva-fp-modal').getByRole('button',{name:'发送',exact:true}).click();await page.locator('.eva-fp-modal').waitFor({state:'detached'});
