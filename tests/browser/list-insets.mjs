...
test('list insets: scrollbar-independent symmetry, controls and interaction across rail widths',async()=>{
 ...
 try{
  ...
  await scroll.hover();await page.waitForTimeout(1500);assert.equal(await page.locator('.eva-list-scroll-track').evaluate(n=>getComputedStyle(n).opacity),'0','idle scrollbar hides');await scroll.evaluate(n=>n.scrollTop=150);await page.waitForFunction(()=>getComputedStyle(document.querySelector('.eva-list-scroll-track')).opacity==='1');assert.equal(await page.locator('.eva-list-scroll-track').evaluate(n=>getComputedStyle(n).opacity),'1','scroll reveals scrollbar');
