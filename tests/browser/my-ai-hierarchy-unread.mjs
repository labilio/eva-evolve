...
test('我的 Agent：默认层级、分层未读与已读回收保持一致', async () => {
  ...
  try {
    ...
    const fixture = await page.evaluate(() => {
      ...
      return { customGroupId, customGroupName, newMemberId, newMemberName, extraThreadId };
    });
    ...
    await teamInfo.getByRole('button', { name: '编辑 AI 团队', exact: true }).click();
    const groupEditor = page.locator('.semi-modal').filter({ hasText: '编辑 AI 团队' });
