...
test('我的 Agent：默认层级、分层未读与已读回收保持一致', async () => {
  ...
  try {
    ...
    const fixture = await page.evaluate(() => {
      ...
      return { customGroupId, customGroupName, extraThreadId };
    });
    ...
    await customTeam.locator('.eva-ai-team__team-heading').hover();
    await customTeam.getByRole('button', { name: fixture.customGroupName + '的团队操作' }).click();
    await page.locator('.semi-dropdown-item').filter({ hasText: '解散群' }).click();
    ...
    await page.reload();
    await page.locator('.eva-ai-team').waitFor();
    assert.equal(await page.locator(`.eva-ai-team__team-button[aria-label="进入团队会话 ${fixture.customGroupName}"]`).count(), 0, '刷新后已解散团队不会恢复');
