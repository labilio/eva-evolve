...
test('个人 Eva 助理与对话只渲染在路由页中间栏', () => {
  ...
  assert.match(imPatch, /const openLatestConversation = \(id,sessions\) => choose\(id,sessions\[0\]\?\.id\|\|null\)/);
  assert.match(imPatch, /'aria-label':'进入最近会话 '\+i\.name.+onClick:\(\)=>openLatestConversation\(i\.id,sessions\)/s);
...
test('我的 AI 小队和身份会话均默认预览最新三条并按需展开', () => {
  ...
  assert.match(identityItem, /'aria-label':'进入最近会话 '\+i\.name.+onClick:\(\)=>openLatestConversation\(i\.id,sessions\)/s);
  ...
  assert.match(employeeItem, /'aria-label':'进入最近会话 '\+item\.name.+onClick:\(\)=>openLatestConversation\(item\.id,sessions\)/s);
