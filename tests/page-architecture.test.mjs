...
test('个人 Eva 助理与对话只渲染在路由页中间栏', () => {
  ...
  assert.match(imPatch, /source\.fixedGroupActions=\{onEditMembers:\(\)=>openGroupMembersEditor\(selectedGroup\),onRename:name=>groupStore\.updateGroup\(selectedGroup\.id,\{name\}\),onUpdateAvatar:avatar=>groupStore\.updateGroup\(selectedGroup\.id,\{avatar\}\),onDissolve:\(\)=>openGroupDissolve\(selectedGroup\)\}/);
  ...
  assert.match(chatSettings, /h\(Row,\{title:'团队头像'.+onClick:\(\)=>setPicker\('avatar'\)/s);
  assert.match(chatSettings, /fixedGroupActions\?\.onUpdateAvatar\?fixedGroupActions\.onUpdateAvatar\(avatar\):update\(\{avatar\}\)/);
  ...
  assert.match(imPatch, /membersOnly\?null:h\('div',\{className:'eva-ai-team-editor__identity'/);
...
test('团队消息和我的 AI 的第二栏使用同一套 GDS 文字层级', () => {
  ...
  assert.match(messageSwitcherCss, /wk-sidebar-tabbar__container\s*\{[^}]*height:\s*36px[^}]*padding:\s*3px/s);
  assert.match(messageSwitcherCss, /wk-sidebar-tabbar__btn\s*\{[^}]*min-height:\s*30px/s);
