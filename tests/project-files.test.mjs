...
test('失去空间权限后置顶文件不泄露，重新加入后恢复显示',()=>{const {members,files}=setup();const id=files.upload('a','p',{name:'权限资料.pdf',size:1});files.setPinned('b',id,true);members.remove('p','a','b');assert.equal(files.pinnedFiles('b').length,0);members.addMember('p','a','b');assert.equal(files.pinnedFiles('b')[0].id,id);});
