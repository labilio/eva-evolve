...
test('任务附件保存为项目文件独立副本并保留任务来源',()=>{const {files}=setup();const task={id:'issue-104',identifier:'SC-104',title:'分析核心品类采购成本偏差',workspace_id:'p'},attachment={id:'att-cost',name:'核心品类采购成本偏差.csv',size:736,extension:'csv',content_type:'text/csv',version:1,url:'blob:task-cost'};const id=files.saveTaskAttachment('b','p',attachment,task);const saved=files.list('p','b').find(item=>item.id===id);assert.equal(saved.name,attachment.name);assert.equal(saved.url,attachment.url);assert.equal(saved.source.type,'task-copy');assert.equal(saved.taskArtifact.taskId,task.id);assert.equal(files.relationsFor(saved,'b')[0].id,task.id);assert.equal(files.findTaskAttachment('b','p',attachment,task).id,id);assert.equal(files.saveTaskAttachment('b','p',attachment,task),id);assert.equal(files.list('p','b').filter(item=>item.taskArtifact?.taskId===task.id).length,1);});
test('任务附件只能保存到所属且有上传权限的项目文件库',()=>{const {members,files}=setup();members.createProject('other','其他项目','c',[]);const task={id:'issue-104',identifier:'SC-104',title:'成本偏差',workspace_id:'p'},attachment={id:'att-cost',name:'成本偏差.csv'};assert.throws(()=>files.saveTaskAttachment('a','other',attachment,task),/任务不属于当前项目/);assert.throws(()=>files.saveTaskAttachment('c','p',attachment,task),/当前角色无此操作权限/);assert.equal(files.findTaskAttachment('c','p',attachment,task),null);});
...
test('文件库使用与项目文件一致的单行工具栏并在两个入口展示完整创建时间',()=>{
  ...
  assert.match(driveHTML,/data-drive-action="new-folder"><span>新建文件夹<\/span><\/button>/);
  assert.match(drive,/data-drive-action="toggle-external-add"><span>添加外部资源<\/span><span class="eva-lucide-host">/);
  assert.match(driveHTML,/data-drive-action="upload-file"><span>上传本地文件<\/span><\/button>/);
  assert.match(project,/setDialog\(\{type:'new-folder',value:''\}\)\},'新建文件夹'\)/);
...
test('项目专家和项目专员按 Editor 写入任务产出，文件仍归属项目文件库',()=>{const {files}=setup();const actor='project-agent:p';assert.equal(files.role('p',actor),'editor');files.upload(actor,'p',{name:'任务总结.md',size:120});const output=files.list('p','a').find(entry=>entry.name==='任务总结.md');assert.equal(output.projectId,'p');assert.equal(output.creator,'项目 · 项目管家');assert.equal(files.can('trash','p',actor),false);});
test('置顶是账号级偏好，同一文件对其他账号保持独立',()=>{const {files}=setup();const id=files.upload('a','p',{name:'常用报告.pdf',size:10});assert.equal(files.isPinned('a',id),false);assert.equal(files.setPinned('a',id,true),true);assert.equal(files.setPinned('a',id,true),true);assert.equal(files.isPinned('a',id),true);assert.equal(files.isPinned('b',id),false);assert.equal(files.list('p','a').find(item=>item.id===id).pinned,true);assert.equal(files.list('p','b').find(item=>item.id===id).pinned,false);assert.equal(files.pinnedFiles('a')[0].id,id);assert.equal(files.pinnedFiles('b').length,0);});
test('置顶文件和文件夹优先排序，未置顶文件夹位于普通文件之前',()=>{const {members,files,sharing}=setup();const older=files.upload('a','p',{name:'较早置顶.pdf',size:1});const newer=files.upload('a','p',{name:'较新置顶.pdf',size:1});const pinnedFolder=files.createFolder('a','p','置顶文件夹');const normalFolder=files.createFolder('a','p','普通文件夹');const plain=files.upload('a','p',{name:'普通文件.pdf',size:1});const seeded=sharing.create(members,files.snapshot(),undefined,[],[{actorId:'a',fileId:older,pinnedAt:'2026-09-08T09:00:00.000Z'},{actorId:'a',fileId:newer,pinnedAt:'2026-09-08T10:00:00.000Z'},{actorId:'a',fileId:pinnedFolder,pinnedAt:'2026-09-08T11:00:00.000Z'}]);const order=seeded.sortEntries('a',seeded.list('p','a')).map(item=>item.id);assert.equal(JSON.stringify(order.slice(0,3)),JSON.stringify([pinnedFolder,newer,older]));assert.ok(order.indexOf(normalFolder)<order.indexOf(plain));});
test('置顶随重命名和移动保留，副本不继承，回收站隐藏并在恢复后重新出现',()=>{const {files}=setup();const folderId=files.createFolder('a','p','归档');const id=files.upload('a','p',{name:'置顶资料.docx',size:1});files.setPinned('a',id,true);files.rename('a',id,'置顶资料-v2.docx');files.move('a',id,folderId);assert.equal(files.pinnedFiles('a')[0].name,'置顶资料-v2.docx');const copyId=files.copy('a',id,folderId);assert.equal(files.isPinned('a',copyId),false);files.trash('a',id);assert.equal(files.pinnedFiles('a').some(item=>item.id===id),false);files.restore('a',id);assert.equal(files.pinnedFiles('a').some(item=>item.id===id),true);files.trash('a',id);files.removeForever('a',id);assert.equal(files.isPinned('a',id),false);});
test('文件夹和文件快捷方式可以置顶，回收站条目不能置顶',()=>{const {files}=setup();const folderId=files.createFolder('a','p','资料夹');files.setPinned('a',folderId,true);assert.equal(files.pinnedFiles('a').some(item=>item.id===folderId&&item.type==='folder'),true);files.trash('a',folderId);assert.equal(files.pinnedFiles('a').some(item=>item.id===folderId),false);assert.throws(()=>files.setPinned('a',folderId,true),/回收站中的文件或文件夹不能置顶/);files.restore('a',folderId);assert.equal(files.pinnedFiles('a').some(item=>item.id===folderId),true);const sourceId=files.upload('a','p',{name:'源文件.pdf',size:1});const shortcutId=files.createShortcut('a',sourceId,'personal:a');files.setPinned('a',shortcutId,true);assert.equal(files.pinnedFiles('a').some(item=>item.id===shortcutId&&item.type==='shortcut'),true);});
test('失去文件库权限后置顶文件不泄露，重新加入后恢复显示',()=>{const {members,files}=setup();const id=files.upload('a','p',{name:'权限资料.pdf',size:1});files.setPinned('b',id,true);members.remove('p','a','b');assert.equal(files.pinnedFiles('b').length,0);members.addMember('p','a','b');assert.equal(files.pinnedFiles('b')[0].id,id);});
test('置顶偏好使用独立本地存储并可在重新加载后恢复',()=>{const values=new Map(),localStorage={getItem:key=>values.get(key)||null,setItem:(key,value)=>values.set(key,value)};const window={localStorage};loadIdentityEnvironment(window);for(const file of ['009-2-membership.js','009-1-file-sharing.js'])vm.runInNewContext(fs.readFileSync(new URL('../prototype/'+file,import.meta.url),'utf8'),{window});const members=window.EvaMembership.create({people:[{id:'a'}]});members.createProject('p','项目','a',[]);const first=window.EvaFileSharing.bootstrap(members);const id=first.upload('a','p',{name:'持久化文件.pdf',size:1});first.setPinned('a',id,true);const payload=JSON.parse(values.get('eva:file-pins:v1'));assert.equal(payload.schema,1);assert.equal(payload.pins[0].fileId,id);const second=window.EvaFileSharing.bootstrap(members);assert.equal(second.isPinned('a',id),true);assert.equal(second.pinnedFiles('a')[0].id,id);});
...
test('v6 文件数据升级到 v7 时丢弃共享空间记录，仅保留仍存在资源的置顶偏好',()=>{const values=new Map([['eva:file-store:v6',JSON.stringify({schema:6,records:[{id:'shared-file',spaceId:'shared:test',area:'shared',name:'旧共享文件.pdf',type:'blob',size:1},{id:'project-file',spaceId:'p',projectId:'p',area:'project',name:'项目文件.pdf',type:'blob',size:1}],sharedSpaces:[{id:'shared:test'}],externalLinksDemoV1:true})],['eva:file-pins:v1',JSON.stringify({schema:1,pins:[{actorId:'a',fileId:'project-file',pinnedAt:'2026-09-08T10:00:00.000Z'},{actorId:'a',fileId:'shared-file',pinnedAt:'2026-09-08T11:00:00.000Z'}]})]]);const localStorage={getItem:key=>values.get(key)||null,setItem:(key,value)=>values.set(key,value),removeItem:key=>values.delete(key)};const window={localStorage};loadIdentityEnvironment(window);for(const file of ['009-2-membership.js','009-1-file-sharing.js'])vm.runInNewContext(fs.readFileSync(new URL('../prototype/'+file,import.meta.url),'utf8'),{window});const members=window.EvaMembership.create({people:[{id:'a'}]});members.createProject('p','项目','a',[]);const files=window.EvaFileSharing.bootstrap(members);const saved=JSON.parse(values.get('eva:file-store:v7')),pins=JSON.parse(values.get('eva:file-pins:v1'));assert.equal(saved.schema,7);assert.equal(saved.records.some(item=>item.id==='shared-file'),false);assert.equal(saved.records.some(item=>item.id==='project-file'),true);assert.equal(files.snapshot().some(item=>item.area==='shared'),false);assert.equal(files.pinnedFiles('a')[0].id,'project-file');assert.deepEqual(pins.pins.map(item=>item.fileId),['project-file']);});
...
test('两个文件入口用挤压式右侧栏预览、用弹窗展示文件详情',()=>{
  const drive=fs.readFileSync(new URL('../prototype/020-mode-layer.js',import.meta.url),'utf8');
  const project=fs.readFileSync(new URL('../prototype/009-1-project-files-ui.js',import.meta.url),'utf8');
  const styles=fs.readFileSync(new URL('../prototype/050-file-library.css',import.meta.url),'utf8');
  const layout=fs.readFileSync(new URL('../prototype/044-final-layout-convergence.js',import.meta.url),'utf8');
  for(const source of [drive,project]){
    assert.match(source,/eva-file-preview-sidebar/);
    assert.match(source,/data-eva-file-preview-resizer/);
    assert.match(source,/eva-file-detail-dialog/);
    assert.doesNotMatch(source,/eva-drive-preview-dialog|eva-project-files__inspector/);
  }
  ...
  assert.match(layout,/data-eva-file-preview-resizer/);
  const previewRule=styles.match(/\.eva-file-preview-sidebar\s*\{([\s\S]*?)\}/)?.[1]||'';
