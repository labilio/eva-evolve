...
test('负责人和群主退出前必须转让；无其他人类时解散普通群',()=>{const s=setup();s.createProject('p','项目','a',[]);assert.throws(()=>s.remove('p','a','a'));s.addMember('p','a','b');s.transfer('p','a','b');s.remove('p','a','a');assert.equal(s.canRead('p','a'),false);s.createGroup('g','小群','p','b',[]);assert.throws(()=>s.remove('g','b','b'));s.dissolveGroup('g','b');assert.equal(s.canRead('g','b'),false);});
