...
test('创建项目可同事务拉入联系人与自己分身且不代带他人分身',()=>{const s=setup();s.createProject('p','项目','a',['aa'],'共同目标',['b','c','b']);assert.equal(s.members('p').length,5);assert.equal(s.canRead('all:p','b'),true);assert.equal(s.canRead('p','bb'),false);assert.equal(s.canRead('p','c'),true);assert.throws(()=>s.createProject('p2','项目2','a',[],'目标',['nope']));assert.equal(s.snapshot().projects.p2,undefined);assert.throws(()=>s.createProject('p3','项目3','a',['bb']));});
...
test('群主退出必须指定接任者，转让后离开父群及子区',()=>{const s=setup();s.createProject('p','项目','a',['aa']);s.addMember('p','a','b');s.addClone('p','b','bb');s.createGroup('g','小群','p','a',['aa']);s.addMember('g','a','b');s.addClone('g','b','bb');s.createThread('t','g',{name:'子区'},'a');assert.throws(()=>s.leaveGroup('g','a'));const result=s.leaveGroup('g','a','b');assert.equal(result.type,'left');assert.equal(result.successorId,'b');assert.equal(s.snapshot().groups.g.ownerId,'b');assert.equal(s.canRead('g','a'),false);assert.equal(s.canRead('t','aa'),false);assert.equal(s.canRead('t','b'),true);assert.equal(s.canRead('t','bb'),true);});
test('群主没有可转让对象时不能退出群聊',()=>{const s=setup();s.createProject('p','项目','a',['aa']);s.createGroup('g','单人群','p','a',['aa']);s.createThread('t','g',{name:'子区'},'a');assert.throws(()=>s.leaveGroup('g','a'));assert.equal(s.snapshot().groups.g.ownerId,'a');assert.equal(s.canRead('t','a'),true);});
