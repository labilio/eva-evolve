...
test('移除人类级联移除分身和普通群关系；全员群不能独立修改',()=>{const s=setup();s.createProject('p','项目','a',[]);s.addMember('p','a','b');s.addClone('p','b','bb');s.createGroup('g','小群','p','a',[]);s.addMember('g','a','b');s.addClone('g','b','bb');assert.throws(()=>s.remove('all:p','a','b'));s.remove('p','a','b');assert.equal(s.canRead('g','bb'),false);assert.equal(s.canRead('p','b'),false);s.addMember('p','a','b');assert.equal(s.canRead('g','b'),false);assert.equal(s.canRead('p','bb'),false);});
...
test('群主直接退出时自动转让给稳定顺序中的其他人类成员',()=>{const s=setup();s.createProject('p','项目','a',['aa']);s.addMember('p','a','b');s.addClone('p','b','bb');s.createGroup('g','小群','p','a',['aa']);s.addMember('g','a','b');s.addClone('g','b','bb');s.createThread('t','g',{name:'子区'},'a');const result=s.leaveGroup('g','a');assert.equal(result.type,'left');assert.equal(result.successorId,'b');assert.equal(s.snapshot().groups.g.ownerId,'b');assert.equal(s.canRead('g','a'),false);assert.equal(s.canRead('t','aa'),false);assert.equal(s.canRead('t','b'),true);assert.equal(s.canRead('t','bb'),true);});
test('群主没有其他可接任人类成员时退出并解散父群及全部子区',()=>{const s=setup();s.createProject('p','项目','a',['aa']);s.createGroup('g','单人群','p','a',['aa']);s.createThread('t','g',{name:'子区'},'a');const result=s.leaveGroup('g','a');assert.equal(result.type,'dissolved');assert.equal(s.snapshot().groups.g,undefined);assert.equal(s.snapshot().threads.t,undefined);assert.equal(s.canRead('g','a'),false);assert.equal(s.canRead('t','a'),false);});
...
test('演示身份只允许已激活人类；重新添加不恢复分身与角色',()=>{const s=setup();s.setActor('b');assert.equal(s.snapshot().actorId,'b');assert.throws(()=>s.setActor('bb'));s.createProject('p','项目','a',[]);s.addMember('p','a','b');s.addClone('p','b','bb');s.setAdmin('p','a','b',true);s.remove('p','a','b');assert.equal(s.canRead('p','b'),false);s.addMember('p','a','b');assert.equal(s.canRead('p','bb'),false);assert.equal(s.manager('p','b'),false);});
...
test('项目群列表派生唯一全员群，普通群按成员访问且人数实时同步',()=>{const s=setup();s.createProject('p','项目','a',['aa']);s.createGroup('g','普通群','p','a',[]);s.addMember('p','a','b');s.addClone('p','b','bb');assert.equal(s.channels('p','b',[]).length,1);assert.equal(s.channels('p','b',[])[0].id,'all:p');assert.equal(s.channels('p','b',[])[0].humanCount,2);assert.equal(s.channels('p','b',[])[0].cloneCount,2);assert.equal(s.channels('p','c',[]).length,0);s.addMember('g','a','b');assert.equal(s.channels('p','b',[]).length,2);s.remove('p','a','b');assert.equal(s.channels('p','a',[])[0].humanCount,1);assert.equal(s.channels('p','b',[]).length,0);});
...
test('@所有人只记录人类通知目标；删除子区后不可再读取',()=>{const s=setup();s.createProject('p','项目','a',['aa']);s.createThread('t','all:p',{name:'子区'},'a');s.sendMessage('t','a','@所有人 请确认');assert.deepEqual(Array.from(s.messagesFor('t','a')[0].notifiedHumanIds),['a']);s.updateThread('t',{deleted:true},'a');assert.equal(s.canRead('t','a'),false);assert.equal(s.messagesFor('t','a').length,0);});
